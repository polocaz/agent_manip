...
- `cli.rs` — subcommand implementations, plus the `lsiagent.cfg` trace-level parsing/editing (`read_log_level`/`set_log_level`, unit-tested) and `where` (log site → agent source checkout: `$LSMAN_AGENT_SRC`, `~/src/systrack/Agent`, `~/src/systrack-col1710/Agent`). `lsman db` passes raw SQL through `sqlite3 -readonly`; `db --table <name>` prints newest rows with string-IDs resolved, `db --find <str>` searches the string tables. Log reading is **streamed/bounded** — the main agent log can be hundreds of MB; never `read_to_string` an agent log.
- `db.rs` — shared read-only agent-DB access (`sqlite3 -readonly -json`) and **SysTrack string-ID resolution**: inventory/execution tables store integer string-IDs, and `resolve_string_ids` batch-maps them to `SASTR`/`SASTRUSER` values (column heuristic in `string_id_column`, unit-tested). Resolution is an annotation — lookup failures degrade to unresolved, never to an error.
...
- DB `<base>/database/collect.sqlite3` — always open read-only (`sqlite3 -readonly`) against a live agent. `LSMAN_DB=<path>` points all DB features at an offline snapshot instead (e.g. a support drop's `collect_copy.sqlite3`).
- Inventory/execution tables hold integer string-IDs, not names: `SASTR (STRINGID, STRVALUE)` = system scope, `SASTRUSER` = user scope (account names). `SASFW.PACKAGE/PUB/DISPLAYVER`, `SAAPP.APP_ID/PATH_ID/CMDLINE_ID`, `SASFWSYS.APP_ID` join SASTR; `*.ACCOUNT_ID` joins SASTRUSER. `RECID`/`PROCESS_ID`/`SESSION_ID`/`THREAD_ID` are **not** string-IDs (`db.rs` deny list). Zero SASTR/SASTRUSER matches for a string = nothing on the endpoint can reference it (conclusive absence).
